				...
				case "upd":
					...
					for (const i of mod.settings.bosses) {
						...
						if (i.logTime == 0) {
							...
							MSG.chat(MSG.RED(i.name) + " ожидаемое время " + MSG.TIP(getTime(nextTime)))
						} else {
							MSG.chat(MSG.RED(i.name) + " просрочен " + MSG.GRY(getTime(nextTime)))
						}
					}
					// break
				// case "神秘":
		...
		switch (sysMsg.id) {
			case 'SMT_FIELDBOSS_APPEAR':
				...
				if (boss) {
					...
					console.log(new Date().toTimeString() + " обновлять " + boss.name)
